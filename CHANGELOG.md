...
## 2025-12-16 - 考试倒计时和Excel导出优化

### 用户体验改进
- ✨ **考试倒计时日期限制**
  - 设置考试日期时，只能选择今天及以后的日期
  - 过去的日期自动禁用，无法选择
  - 已过期的日期仍然显示，但编辑时无法选择过去的日期
  - 避免用户误选过去的日期

- ✨ **Excel导出增强**
  - 导出"历次考试各模块详细数据表"时，包含底部的汇总统计
  - 新增导出内容：总时长、总答对、总题量、总正确率、得分
  - 数据更完整，便于分析
  - 与页面显示保持一致
...
const summaryRowsData = [
  { key: 'summary_time', module_name: '总时长', ... },
  { key: 'summary_correct', module_name: '总答对', ... },
  { key: 'summary_total', module_name: '总题量', ... },
  { key: 'summary_accuracy', module_name: '总正确率', ... },
  { key: 'summary_score', module_name: '得分', ... }
];
```
...
2. 查看"历次考试各模块详细数据表"
3. 点击"导出Excel"按钮
4. ✅ 导出包含所有模块数据和底部汇总统计的Excel文件

### 相关文档
- `docs/考试倒计时和Excel导出优化说明.md` - 详细技术文档
