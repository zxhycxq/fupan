...
### 数据同步问题（已修复）
**问题描述：**
首页"历次考试各模块详细数据表"的数据数量与上传的考试记录数据不一致。

**问题原因：**
- 考试记录列表显示的是 `index_number`（用户设置的索引号）
- Dashboard的模块详细数据表使用的是 `exam_number`（内部编号）进行查询
- 两个字段的值不同，导致显示的期数不一致

**解决方案：**
修改 `getModuleDetailedStats` 函数，使用 `index_number` 而不是 `exam_number` 进行查询和排序，确保：
1. Dashboard表格显示的期数与考试记录列表一致
2. 数据按照用户看到的索引号进行排序
3. 保持整个系统的数据一致性
