...
用户希望将现有的考试记录按照考试日期进行排序，以便查看趋势图等数据分析。按照考试系统的惯例，最早的考试排在前面，最新的考试排在后面。

## 需求

- 按考试日期升序排列（最早的在前面，最新的在后面）
- 没有考试日期的记录排在最后
- 更新数据库中的 `sort_order` 字段
- 修复按钮内边距问题，使其与其他按钮保持一致
...
- `exam_date ASC NULLS LAST` 确保日期升序排列（最早的在前）
...
**问题：**
- "保存排序"、"取消排序"、"上传新记录"按钮使用了 `size="small"` 属性
- 导致按钮内边距过小，与其他页面的按钮样式不一致

**解决方案：**
- 移除所有按钮的 `size="small"` 属性
- 让按钮使用默认大小，保持与其他页面一致的视觉效果
...
使用 SQL 直接执行迁移脚本，成功更新了所有记录的 `sort_order` 字段。

## 验证结果

### 前10条记录（按sort_order排序）
...
2. 第二排序：`exam_date ASC NULLS LAST`
   - 日期升序排列（最早的在前）
   - NULL值排在最后
