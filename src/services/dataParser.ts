...
export function parseExamData(
  ...
  for (const module of moduleStructure) {
    ...
    if (!moduleMatch) {
      ...
      // 关键改进：
      // 1. 支持"总题数"和"共计"
      // 2. 关键词和数字之间可以有1-10个空格
      // 3. 字段之间可以有1-10个空格
      // 4. 答对后面可能是"题"或"道"
      // 5. 用时支持"X秒"、"X分"、"X分Y秒"
      const webPattern = `${module.name}[\\s\\S]{0,150}` +  // 增加搜索范围
        `(?:总题数|共计)[：:\\s]{1,10}(\\d+)[\\s]*(?:题|道)[\\s]{1,10}` +  // 总题数 X题/道
        `答对[：:\\s]{1,10}(\\d+)[\\s]*(?:题|道)[\\s]{1,10}` +  // 答对 Y题/道
        `正确率[：:\\s]{1,10}(\\d+)%[\\s]{1,10}` +  // 正确率 Z%
        `用时[：:\\s]{1,10}(\\d+)(?:[\\s]*(\\d+))?[\\s]*(?:秒|分)`;  // 用时 W秒 或 W分 或 W分X秒
    ...
    for (const childName of module.children) {
      ...
      if (!childMatch) {
        ...
        const childWebPattern = `${childName}[\\s\\S]{0,150}` +  // 增加搜索范围
          `(?:总题数|共计)[：:\\s]{1,10}(\\d+)[\\s]*(?:题|道)[\\s]{1,10}` +  // 总题数 X题/道
          `答对[：:\\s]{1,10}(\\d+)[\\s]*(?:题|道)[\\s]{1,10}` +  // 答对 Y题/道
          `正确率[：:\\s]{1,10}(\\d+)%[\\s]{1,10}` +  // 正确率 Z%
          `用时[：:\\s]{1,10}(\\d+)(?:[\\s]*(\\d+))?[\\s]*(?:秒|分)`;  // 用时 W秒 或 W分 或 W分X秒
