...
ADD COLUMN IF NOT EXISTS vip_type TEXT CHECK (vip_type IN ('quarter', 'year'));

-- 添加注释
COMMENT ON COLUMN user_vip.vip_type IS 'VIP类型：quarter=季度会员，year=年度会员';

-- 更新现有记录（根据 duration 推断类型）
UPDATE user_vip 
SET vip_type = CASE 
  WHEN EXTRACT(MONTH FROM (vip_end_date - vip_start_date)) <= 4 THEN 'quarter'
  ELSE 'year'
END
WHERE vip_type IS NULL AND is_vip = TRUE;

-- 更新 admin_activate_vip 函数，添加 vip_type 参数
...
BEGIN
  ...
  -- 确定 VIP 类型
  v_vip_type := CASE WHEN p_duration_months = 3 THEN 'quarter' ELSE 'year' END;
  ...
  -- 开通会员（使用 UPSERT）
  INSERT INTO user_vip (user_id, is_vip, vip_type, vip_start_date, vip_end_date)
  VALUES (p_user_id, TRUE, v_vip_type, NOW(), v_vip_end_date)
  ON CONFLICT (user_id) 
  DO UPDATE SET
    is_vip = TRUE,
    vip_type = v_vip_type,
...
BEGIN
  ...
  IF NOT FOUND THEN
    INSERT INTO user_vip (user_id, is_vip, vip_type, vip_start_date, vip_end_date)
    VALUES (p_user_id, TRUE, v_vip_type, NOW(), v_new_end_date);
  END IF;
