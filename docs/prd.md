...
### 1.2 系统描述
一个专门用于分析和管理模拟考试成绩的Web系统，通过上传考试截图，利用文字识别技术提取成绩数据，进行多维度分析和可视化展示，同时提供模考计时工具帮助用户精准记录各模块答题时间，帮助用户全面了解自身学习状况，制定针对性提升计划。系统采用会员制，用户需注册登录后方可使用全部功能。系统提供季度VIP和年度VIP两种会员服务，VIP用户享有更多高级功能权限。\n
## 2. 核心功能
...
- 用户类型（普通用户或VIP用户）\n- VIP到期时间（VIP用户显示，格式：YYYY-MM-DD）\n- 订单功能预留接口（为后续会员功能扩展做好准备）\n
#### 2.1.5 用户名设置规则

- 用户在注册时设置用户名（昵称）\n- 系统实时校验用户名是否符合规则，并显示相应提示
- 校验规则：\n  - 用户名仅允许使用字母、数字和下划线
  - 禁用易被猜测或攻击的通用用户名（如admin、root、user、test、123456等）\n  - 用户名长度限制为3-16个字符
- 实时反馈提示：\n  - ✅ 该用户名可用
  - ❌ 用户名只能包含字母、数字和下划线
  - ❌ 该用户名已被占用
  - ❌ 用户名长度必须在3-16个字符之间
...
在个人中心页面的账户信息模块中新增用户类型显示，如用户为普通用户，则显示〖开通VIP〗按钮。用户点击〖开通VIP〗按钮后，系统弹出会员购买支付弹窗：\n
**弹窗内容：**\n
- 标题：〖选择会员套餐〗\n- 套餐选项：\n  + 季度VIP：3个月会员，价格XX元
  + 年度VIP：12个月会员，价格XX元
- 每个套餐展示对应权益说明
...
**普通用户限制：**\n
- 普通用户最多只能保存3条考试记录
- 当用户已有3条记录时，尝试上传新记录时，系统弹出提示：〖您已达到普通用户上传记录上限（3条），请删除旧记录后再上传，或开通VIP享受无限上传权限〗\n- 提示弹窗包含〖删除旧记录〗和〖开通VIP〗两个操作入口

**VIP用户权限：**\n
- VIP用户无上传记录数量限制，可无限制上传考试记录

#### 2.2.7 导出Excel功能（VIP专属）\n
**功能入口：**\n
- 在〖各模块分析〗页面右上角或合适位置添加〖导出Excel〗按钮
- 按钮旁显示VIP图标标识
- 在考试记录列表页面右上角添加〖导出Excel〗按钮
- 按钮旁显示VIP图标标识
...
**非VIP用户交互：**\n
- 普通用户点击〖导出Excel〗按钮时，系统弹出VIP权益对比弹窗（参考2.2.4）\n- 弹窗提示：〖导出Excel功能为VIP专属功能，开通VIP即可使用〗\n
### 2.3 成绩数据录入
...
- 用户需输入上传图片对应的考试名称
- 后端根据索引项顺序保存每次考试数据
- 普通用户上传时，系统校验当前考试记录数量，若已达3条上限，则弹出提示并阻止上传
...
- 普通用户提交表单时，系统校验当前考试记录数量，若已达3条上限，则弹出提示并阻止提交
