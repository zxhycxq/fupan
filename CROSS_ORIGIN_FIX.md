...
1. **添加 document 对象访问保护**
   - 检查 `typeof document !== 'undefined'`
   - 为 `document.createElement('canvas')` 添加 try-catch
   - 在所有可能失败的步骤添加错误处理

2. **提供降级方案**
   - 如果 document 不可用，跳过压缩，使用原图
   - 如果 canvas 创建失败，使用原图
   - 如果图片加载失败，使用原图
   - 如果文件读取失败，使用原图
   - 确保图片上传功能在任何环境中都能工作
...
在跨域环境中，`document.createElement('canvas')` 可能会失败。修复策略：
...
3. **降级方案**
   - 任何步骤失败都返回原图
   - 使用 `resolve(file)` 而非 `reject(error)`
   - 确保上传流程不会中断
